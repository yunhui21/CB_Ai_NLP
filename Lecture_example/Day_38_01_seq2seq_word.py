...
def show_seq2seq(enc_inputs, dec_inputs, dec_target, vocab, char2idx):
    ...
    # 문제
    # 잘 예측하는지 결과를 출력하세요 (기존 데이터에서 하나를 골라 검증하세요)
    ...
    print(results)
    # [['우리에게', '영웅은', '필요하지', '않아', '_EOS_'],
    # ['나는', '파랑색을', '진짜', '좋아해', '_EOS_']]
    ...
    # ['우리에게 영웅은 필요하지 않아', '나는 파랑색을 진짜 좋아해']
...
# 앞에서 만든 글자 버전의 데이터를 단어 버전에 적용시키는 코드
# def convert(s):
#     return ' '.join(list(s))
#
# data = [(convert('food'), convert('음식')), (convert('wood'), convert('나무')),
#         (convert('blue'), convert('파랑')), (convert('lamp'), convert('전구')),
#         (convert('wind'), convert('바람')), (convert('hero'), convert('영웅'))]
