...
def model_fc(train_gen, valid_gen, steps_valid, n_features):
    ...
    save_history(history, 'data/jena_1_fc.pickle')
...
def model_gru(train_gen, valid_gen, steps_valid, n_features):
    ...
    history = model.fit_generator(train_gen,
                                  ...
                                  validation_steps=steps_valid,
                                  verbose=2)
    save_history(history, 'data/jena_2_gru.pickle')
...
def model_gru_dropout(train_gen, valid_gen, steps_valid, n_features):
    ...
    history = model.fit_generator(train_gen,
                                  ...
                                  validation_steps=steps_valid,
                                  verbose=2)
    save_history(history, 'data/jena_3_gru_dropout.pickle')


def model_gru_stack(train_gen, valid_gen, steps_valid, n_features):
    model = tf.keras.Sequential()
    model.add(tf.keras.layers.Input([None, n_features]))
    model.add(tf.keras.layers.GRU(32, dropout=0.1, recurrent_dropout=0.5, return_sequences=True))
    ...
    history = model.fit_generator(train_gen,
                                  ...
                                  validation_steps=steps_valid,
                                  verbose=2)
    # save_history(history, 'data/jena_4_gru_stack.pickle')
...
def model_1d_conv(train_gen, valid_gen, steps_valid, n_features):
    ...
    history = model.fit_generator(train_gen,
                                  ...
                                  validation_steps=steps_valid,
                                  verbose=2)
    save_history(history, 'data/jena_6_1d_conv.pickle')
...
def model_1d_conv_rnn(train_gen, valid_gen, steps_valid, n_features):
    ...
    history = model.fit_generator(train_gen,
                                  ...
                                  validation_steps=steps_valid,
                                  verbose=2)
    save_history(history, 'data/jena_7_1d_conv_rnn.pickle')
...
# show_history('data/jena_3_gru_dropout.pickle', '3_gru_dropout')
# show_history('data/jena_4_gru_stack.pickle', '4_gru_stack')
# show_history('data/jena_5_bidirectional.pickle', '5_bidirectional')
# show_history('data/jena_6_1d_conv.pickle', '6_1d_conv')
# show_history('data/jena_7_1d_conv_rnn.pickle', '7_1d_conv_rnn')
...
# 1. 레이어 갯수 확장
# 2. lstm 사용
# 3. rmsprop 대신 adam 사용
# 4. learning rate 조정
# 5. 각각의 모델에 구현된 여러 가지 기법들 동시 적용
# 6. 최종 모델로부터 최고의 순간 포착
